...
class CogVideoXFunInpaintPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        expected_latent_length = (num_frames - 1) // self.vae_scale_factor_temporal + 1
        if video is not None and video_length != num_frames:
            init_video = init_video[:, :, :num_frames]
            video_length = num_frames
            if mask_video is not None:
                mask_video = mask_video[:, :, :num_frames]

        if latents is not None:
            if latents.shape[1] < expected_latent_length:
                raise ValueError(
                    f"Provided latents have {latents.shape[1]} frames, but {expected_latent_length} are required."
                )
            if latents.shape[1] != expected_latent_length:
                latents = latents[:, :expected_latent_length]
        ...
        if mask_video is not None:
            if (mask_video == 255).all():
                ...
                video_length = init_video.shape[2] if init_video is not None else mask_video.shape[2]
        ...
        latent_temporal_window_size = (temporal_window_size - 1) // 4 + 1
